...
export default function MonitorScreen() {
  ...
  const currentProfile = useQuery({ queryKey: ['current-user-profile', config.activeAccountId], queryFn: getCurrentUser, enabled: authenticated && config.authMode === 'password', staleTime: 30000 });
  const defaultAdminProfile = useQuery({ queryKey: ['dashboard-default-admin', config.activeAccountId], queryFn: () => listUsers('', 1, 100), enabled: authenticated && admin && config.authMode === 'admin_key', staleTime: 30000 });
  const refresh = () => { stats.refetch(); trend.refetch(); models.refetch(); if (admin) accounts.refetch(); if (config.authMode === 'password') currentProfile.refetch(); else if (admin) defaultAdminProfile.refetch(); };
  const points = trend.data?.trend ?? []; const data = stats.data; const refreshing = stats.isRefetching || trend.isRefetching || models.isRefetching || accounts.isRefetching || currentProfile.isRefetching || defaultAdminProfile.isRefetching;
  ...
  const cards = admin ? [
    ...
    ['剩余额度', money(profile?.balance), WalletCards, '/monitor'], ['API 密钥', compact(data?.total_api_keys), KeyRound, '/api-keys'], ['今日请求', compact(data?.today_requests), CheckCircle2, '/usage-logs'], ['总请求', compact(data?.total_requests), Server, '/usage-logs'], ['今日 Token', compact(data?.today_tokens), Coins, '/usage-logs'], ['今日消费', money(data?.today_cost), AlertTriangle, '/usage-logs'],
  ] as const;
  const primaryCards = admin ? cards.slice(0, 4) : cards;
  const finalCards = admin ? cards.slice(4) : [];
  return <SafeAreaView style={{ flex: 1, backgroundColor: dark ? '#0B1220' : '#F4F7FC' }}><ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 50, gap: 12 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor="#2F6DF6" />}>
    <View style={{ marginTop: 16, paddingHorizontal: 4, paddingVertical: 4 }}>
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 16 }}>
        <Text style={{ flex: 1, fontSize: 22, fontWeight: '700', letterSpacing: -0.4, color: dark ? '#F4F7FB' : '#172033' }}>仪表盘</Text>
        <View style={{ flexDirection: 'row', gap: 5 }}>
          {(['24h', '7d', '30d'] as Range[]).map((value) => (
            <Pressable key={value} onPress={() => setRange(value)} style={{ borderRadius: 999, paddingHorizontal: 10, paddingVertical: 7, backgroundColor: range === value ? '#2F6DF6' : dark ? '#273449' : '#E5EBF4' }}>
              <Text style={{ color: range === value ? '#fff' : dark ? '#D5DDEA' : '#607086', fontSize: 10, fontWeight: '800' }}>{value.toUpperCase()}</Text>
            </Pressable>
          ))}
        </View>
      </View>
      <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8} style={{ marginTop: 4, fontSize: 11, lineHeight: 16, color: dark ? '#9EABC0' : '#6B778C' }}>
        {config.user?.email || config.baseUrl} · {admin ? '管理员' : '普通用户'}
      </Text>
    </View>
    ...
    {finalCards.length ? <View style={{ flexDirection: 'row', gap: 9 }}>{finalCards.map(([label, value, Icon, route]) => <Pressable key={label} onPress={() => router.push(route as never)} style={{ flex: 1, borderRadius: 18, padding: 12, backgroundColor: dark ? '#111827' : '#fff', borderWidth: 1, borderColor: dark ? '#273449' : '#E2E9F3' }}><View style={{ flexDirection: 'row', alignItems: 'center' }}><View style={{ width: 32, height: 32, borderRadius: 11, alignItems: 'center', justifyContent: 'center', backgroundColor: dark ? '#172C55' : '#EAF2FF' }}><Icon size={16} color="#2F6DF6" /></View><Text numberOfLines={1} style={{ flex: 1, marginLeft: 7, color: dark ? '#9EABC0' : '#718096', fontSize: 10 }}>{label}</Text></View><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.75} style={{ marginTop: 10, color: dark ? '#F4F7FB' : '#172033', fontSize: 18, fontWeight: '800' }}>{value}</Text></Pressable>)}</View> : null}
